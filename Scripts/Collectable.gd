...
@onready var gfx: Sprite2D = get_child(1)
...
func _ready() -> void:
	...
	else: shadow.modulate = Color(0.0, 0.0, 0.0, 0.235)
	rotation = GLOBALS.random_rotation(360)
	shadow.global_position = gfx.global_position + Vector2(0.35,0.35)
...
func _spawn_countdown() -> void:
	...
	_tween.tween_property(self,"scale",Vector2(0.6,0.6),_land_time)
	_tween.tween_property(self,"modulate",Color.WHITE,_land_time/3).set_delay(_land_time/10)
	_tween.tween_property(shadow,"modulate",Color(0.0, 0.0, 0.0, 0.235),_land_time/2).set_delay(_land_time/2)
	#_tween.tween_property(shadow,"global_position",gfx.global_position + shadow.scale,_land_time)
